...
function xmldb_assignsubmission_upgrade($oldversion) {
    ...
    if ($oldversion < 2026040600) {
        // Define table assignsubmission_mapping to be created.
        $table = new xmldb_table('assignsubmission_mapping');

        // Adding fields to table assignsubmission_mapping.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('course', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        $table->add_field('studentid', XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, null);
        $table->add_field('studentname', XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, null);

        // Adding keys to table assignsubmission_mapping.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        // Adding indexes to table assignsubmission_mapping.
        $table->add_index('course', XMLDB_INDEX_NOTUNIQUE, array('course'));

        // Conditionally launch create table for assignsubmission_mapping.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // assignsubmission savepoint reached.
        upgrade_mod_savepoint(true, 2026040600, 'assignsubmission');
    }
