...
class observer
{
    ...
    public static function assessable_submitted(\mod_assign\event\assessable_submitted $event)
    {
        ...
        foreach ($submission_files as $file) {
            ...
            // If it's an image (like the one the student uploaded), attach base64 content
            if (strpos($mimetype, 'image/') === 0) {
                $content = $file->get_content();
                if ($content !== false) {
                    $base64 = base64_encode($content);
                    $file_info['data'] = "data:{$mimetype};base64,{$base64}";
                }
            }
        ...
        try {
            ...
            if ($response->get_success()) {
                ...
                if (json_last_error() === JSON_ERROR_NONE) {
                    ...
                    \local_orchestrator\memory::update_profile($userid, $courseid, $decoded);
                } else {
